...
bot.on('text', (ctx) => {
  ...
  if (text.includes('/placa')) {
    ...
    bot.telegram.sendMessage(ctx.chat.id, 'As consultas completas estão apenas no melhor site: buscardados.com.br!');
    ...
    bot.telegram.sendMessage(ctx.chat.id, 'Entre no site buscardados.com.br para completar a sua pesquisa de CPF.');
    bot.telegram.sendMessage(ctx.chat.id, 'As consultas completas estão apenas no melhor site: buscardados.com.br!');
    bot.telegram.sendMessage(ctx.chat.id, 'Pague somente pelo pv admin deste grupo ou no whatsapp do site oficial: buscardados.com.br!, o restante são golpistas e você vai perder seu dinheiro!');
  }
});
